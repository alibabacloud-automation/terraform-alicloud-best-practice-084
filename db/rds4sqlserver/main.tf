...
resource "alicloud_db_instance" "this" {
  count                = "${var.use_rds4sqlserver_module ? var.instance_count : 0}"
  tags                 = "${var.tags}"
...
resource "alicloud_db_database" "databases" {
  count         = "${var.use_rds4sqlserver_module ? (var.create_database ? length(var.databases) : 0) : 0}"
...
resource "alicloud_db_account" "account_manager" {
  count       = "${var.use_rds4sqlserver_module ?(var.create_super_account?  1:0 ) : 0}"
  instance_id = "${element(alicloud_db_instance.this.*.id, count.index)}"
...
resource "alicloud_db_account" "account_app" {
  count       = "${var.use_rds4sqlserver_module ? (var.create_app_account?  1:0 ) : 0}"
  instance_id = "${element(alicloud_db_instance.this.*.id, count.index)}"
...
resource "alicloud_db_backup_policy" "backup_policy" {
  count                       = "${var.use_rds4sqlserver_module ? 1 : 0}"
  instance_id                 = "${element(alicloud_db_instance.this.*.id, count.index)}"
...
#   count                 = "${var.use_rds4sqlserver_module ? (var.rw_spliting_enable ? (var.ro_instance_count) : 0) : 0}"
...
#   count             = "${var.use_rds4sqlserver_module ? (var.rw_spliting_enable ? 1 : 0) : 0}"
#   instance_id       = "${element(alicloud_db_instance.this.*.id, count.index)}"
