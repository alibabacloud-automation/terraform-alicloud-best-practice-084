...
resource "alicloud_polardb_cluster" "default" {
  count        = "${var.use_polardb4oracle_module ? var.instance_count : 0}"
...
resource "alicloud_polardb_database" "default" {
  count              = "${var.use_polardb4oracle_module ? (var.create_database ? length(var.databases) : 0) : 0}"
...
resource "alicloud_polardb_account" "account_manager" {
  count       = "${var.use_polardb4oracle_module ?(var.create_super_account?  1:0 ) : 0}"
  db_cluster_id    = "${element(alicloud_polardb_cluster.default.*.id, count.index)}"
...
resource "alicloud_polardb_account" "account_app" {
  count       = "${var.use_polardb4oracle_module ? (var.create_app_account?  1:0 ) : 0}"
  db_cluster_id    = "${element(alicloud_polardb_cluster.default.*.id, count.index)}"
...
resource "alicloud_polardb_backup_policy" "default" {
  count                   = "${var.use_polardb4oracle_module ? 1 : 0}"
  db_cluster_id           = "${element(alicloud_polardb_cluster.default.*.id, count.index)}"
